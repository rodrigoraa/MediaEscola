...
def extrair_boletim_pdf(caminho_pdf):
    ...
    with pdfplumber.open(caminho_pdf) as pdf:
        for pagina in pdf.pages:
            ...
            if linhas_texto:
                linhas_extraidas.extend(linhas_texto)
            else:
                linhas_extraidas.extend(_extrair_por_tabelas(pagina))

    return _remover_duplicados(linhas_extraidas)


def _extrair_por_tabelas(pagina):
    linhas = []
    ...
    for tabela in pagina.extract_tables() or []:
        ...
        for idx, row in enumerate(tabela):
            ...
            if any("Componente Curricular" in _limpar(celula) for celula in row if celula):
                cabecalho_idx = idx
                break
        ...
        for row in tabela[cabecalho_idx + 1 :]:
            ...
            disciplina = registro.get("Componente Curricular", "").strip()

            if not disciplina or _eh_marcador_parada(disciplina):
                continue

            linhas.extend(_montar_linhas_disciplina(aluno, disciplina, registro, situacao))
...
def _extrair_por_texto(texto):
    ...
    for bloco in blocos:
        if "Nome do Aluno" not in bloco or "Componente Curricular" not in bloco:
            continue
        ...
        turma = _buscar_rotulo_no_bloco(bloco, "Turma", ["Componente Curricular", "Turno"])
        ano_letivo = _buscar_rotulo_no_bloco(bloco, "Ano Letivo", ["Ensino Fundamental", "Ano/Fase", "Turno", "Turma"])
        situacao = _buscar_valor_linha(linhas_texto, "Situação Atual do Aluno")

        inicio = _indice_linha(linhas_texto, "Componente Curricular")
        if inicio is None:
            continue
...
def _parsear_linhas_sequenciais(aluno, linhas, situacao, turma="", ano_letivo=""):
    ...
    for linha in linhas:
        ...
        if disciplina and valores_linha:
            fechar_disciplina()
            registros.extend(_montar_linhas_por_valores(aluno, disciplina, valores_linha, situacao, turma, ano_letivo))
            disciplina_atual = None
            valores = []
            continue
...
def _montar_linhas_disciplina(aluno, disciplina, registro, situacao):
    linhas = []
    ...
    for bimestre in BIMESTRES:
        ...
        if nota:
            linhas.append(_linha(aluno, disciplina, bimestre, nota, media_final, situacao))

    if not linhas and media_final:
        linhas.append(_linha(aluno, disciplina, "", "", media_final, situacao))
...
def _eh_cabecalho_tabela(linha):
    texto = linha.lower()
    return linha in BIMESTRES or texto in COLUNAS_FINAIS
